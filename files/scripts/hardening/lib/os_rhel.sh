...
#           is_protected_account, PROTECTED_ACCOUNTS)
#
...
# SAFETY: create zz-<account>-nopasswd for each protected account.
setup_sudoers() {
    log_info "===== [10] sudoers NOPASSWD removal ====="

    # Protect each PROTECTED_ACCOUNTS entry with its own sudoers drop-in
    ...
    if [[ -f /etc/sudoers ]]; then
        ...
        if grep -q 'NOPASSWD' /etc/sudoers; then
            ...
            # Remove NOPASSWD from %wheel lines (skip protected accounts)
            sed -i "${_sed_skip}s/^\(%wheel[[:space:]]\+ALL=(ALL)\)[[:space:]]\+NOPASSWD:[[:space:]]\+ALL/\1 ALL/" /etc/sudoers
            sed -i "${_sed_skip}s/^\(%wheel[[:space:]]\+ALL=(ALL:ALL)\)[[:space:]]\+NOPASSWD:[[:space:]]\+ALL/\1 ALL/" /etc/sudoers
            # Remove NOPASSWD from individual user lines (skip protected accounts)
...
# SAFETY: exclude PROTECTED_ACCOUNTS lines from drift detection
check_sudoers() {
...
kill_other_ssh_sessions() {
    ...
    while IFS= read -r pid; do
        ...
        # Protect accounts listed in PROTECTED_ACCOUNTS
        if is_protected_account "$user"; then
