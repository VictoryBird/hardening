...
# 아웃바운드 정책
# ═══════════════════════════════════════════════════════════════════════════
# restrict: 아래 지정된 포트만 아웃바운드 허용 (리버스 셸 방어, 서버 권장)
# allow:    모든 아웃바운드 허용 (워크스테이션, 특수 환경)
OUTBOUND_POLICY="${OUTBOUND_POLICY:-restrict}"

# restrict 모드일 때 허용할 아웃바운드 포트 (allow 모드에서는 무시됨)
# 기본값은 gtmon 필수 포트 + 규칙서 요구 포트
OUTBOUND_ALLOWED_PORTS="${OUTBOUND_ALLOWED_PORTS:-22/tcp 53/udp 80/tcp 443/tcp 123/udp 8080/tcp 110/tcp 143/tcp 993/tcp 995/tcp 21/tcp}"

# 아웃바운드 ICMP ping 허용 여부
OUTBOUND_ALLOW_ICMP="${OUTBOUND_ALLOW_ICMP:-true}"
...
HARDEN_TUNNEL_DEFENSE="${HARDEN_TUNNEL_DEFENSE:-false}"
