...
hardening_service_allowlist: ""        # 운영자가 수동 입력 (예: "wazuh-agent")
hardening_account_allowlist: ""        # 운영자가 수동 입력 (예: "wazuh ossec")
hardening_ip_forward: "False"
hardening_ssh_password_auth: "no"

# --- 터널링 방어 (기본 false — 실서비스 영향 가능) ---
hardening_harden_tunnel_defense: "false"
...
# SSH 세션 종료 비활성화 (테스트 시)
ansible-playbook playbooks/4002_hardening_apply.yml \
  -i inventories/lab/hosts.yml \
  -e "kill_other_sessions=false"
```

**플레이북이 자동으로 처리하는 항목:**
- `artifacts/hardening_discover/<hostname>.yml` 자동 로드 (include_vars)
- Ansible 접속 계정(`ANSIBLE_ACCOUNT`) → `PROTECTED_ACCOUNTS`에 자동 병합 (config.sh에서 처리)
- IP 포워딩 상태 → `SYSCTL_DISABLE_IP_FORWARD` 자동 결정 (아티팩트 폴백)
- SSH 키 존재 여부 → `SSH_PASSWORD_AUTH` 자동 결정 (아티팩트 폴백)
- 비동기 실행 (async) → 서비스 FD 상속으로 인한 멈춤 방지
- 하드닝 완료 후 비보호 SSH 세션 종료 (`kill_other_sessions: true` 기본값)
...
운영자가 공백 구분으로 직접 입력 (예: `PROTECTED_ACCOUNTS="gt operator"`).

이 목록에 포함된 계정은 하드닝 중:
- 잠금/nologin 변경에서 제외
- sudoers NOPASSWD 제거에서 제외 (zz-<계정>-nopasswd 파일 자동 생성)

Ansible 접속 계정은 플레이북이 `ANSIBLE_ACCOUNT` 환경변수로 전달하여 자동 병합됩니다.

**참고:** 4002 플레이북의 SSH 세션 종료 태스크는 config.sh의 PROTECTED_ACCOUNTS를 직접 읽지 않고, 하드코딩된 `gt` + `ansible_user`만 보호합니다.
...
| 18 | `HARDEN_TUNNEL_DEFENSE` | `false` | 터널링 방어 (실서비스 영향 가능) |
...
4001 discover가 자동으로 리스닝 포트를 수집해 아티팩트에 기록합니다.
비어있으면 SSH 포트만 허용됩니다.
...
2. 포트 추가: discover 아티팩트의 `hardening_allowed_ports` 수정 후 4002 재실행
