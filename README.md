...
ansible-playbook playbooks/4003_hardening_check.yml -i inventories/lab/hosts.yml \
  -e "auto_restore=true" --limit bps_dmz_git
```
...
config.sh에 21개 토글 변수가 있습니다. 항목을 선택적으로 비활성화하려면 config.sh에서 해당 변수를 `false`로 설정하세요:

```bash
# config.sh 예시
HARDEN_SSH="${HARDEN_SSH:-false}"
HARDEN_FIREWALL="${HARDEN_FIREWALL:-false}"
```
